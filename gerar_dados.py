...
def email_from_name(name):
    ...
    for p in name.split():
        ...
        if rng.random() < .5:
            parts.append('.')
    return ''.join(parts).strip('.\'"').strip() + '@' + rng.choice(dominios_email)
...
for i in range(quantidade_profissionais_saude):
    ...
    print(f"({i}, {credencial}, {nome_social}, '{fake_name()}', '{prof}')",
...
for i in range(quantidade_pacientes):
    nome_social = f"'{fake_name()}'" if rng.randint(1, 6) == 6 else 'NULL'
    nome_civil = fake_name()
    if nome_social == 'NULL':
        nome_email = nome_civil
    else:
        nome_email = nome_social
    out.append(f"({i}, {rng.choice(etnias)}, {rng.choice(genero)}, '{rng.choice(tipos_sanguineos) + rng.choice(fatores_rh)}', '{fake.past_datetime().isoformat()}', '{rng.choice(religioes)}', '{str(rng.randint(1, 99999999999)).zfill(11)}', '{str(rng.randint(1, 999999999)).zfill(9)}', '{fake.phone_number()}', '{fake.address()}', {repr(email_from_name(nome_email))}, {nome_social}, {repr(nome_civil)})")
print(',\n'.join(out), end=';\n')
...
print('VALUES')
print(',\n'.join(
    f'({i}, {rng.randrange(0, len(alergias))}, {rng.randrange(0, quantidade_pacientes)})' for i in range(9)) + ';')
...
print('VALUES')
print(',\n'.join(f'({i}, {rng.randrange(0, quantidade_profissionais_saude)}, {repr(rng.choice(especialidades))})' for i in range(
    quantidade_profissionais_saude*3//2)) + ';')
...
print('VALUES')
print(',\n'.join(f'({i}, {rng.randrange(0, quantidade_profissionais_saude)}, {par_consulta_paciente(i)}, {repr(fake.past_datetime().isoformat())}, {repr(fake.text())})' for i in range(
    2*quantidade_pacientes)) + ';')
...
print('VALUES')
print(',\n'.join(
    f'({i}, {rng.randrange(0, 2*quantidade_pacientes)}, {repr(cid())})' for i in range(2*quantidade_pacientes)) + ';')
...
print('VALUES')
print(',\n'.join(f'({i}, {repr(cid())}, {repr(rng.choice(marcas_vacina))}, {repr(rng.choice(nomes_vacina).strip())})' for i in range(
    quantidade_vacinas)) + ';')
...
print('VALUES')
print(',\n'.join(f'({i}, {rng.randrange(0, quantidade_pacientes)}, {rng.randrange(0, quantidade_vacinas)}, {repr(fake.past_date().isoformat())})' for i in range(
    quantidade_pacientes*3)) + ';')
...
nomes_comerciais = [['Tylenol',
                    ...
                    ['Euthyrox',
                     ...
                     'Levothroid',
                     'Unithroid'],
                    ['Zantac', 'Ulceranin', 'Sovran', 'Antak',
                        'Azantac', 'Ranitil', 'Ranid'],
                    ['Zocor',
                     'Sinvastatil',
                     'Sinvaxs',
                     'Sinvax',
                    ...
                    ['Zitromax',
                     ...
                     'Azi',
                     'Azimix'],
                    ['Rivotril', 'Klonopin', 'Paxam',
                        'Clonotril', 'Clonotrilid', 'Clonex'],
                    ['Valium',
                     'Diazepac',
                     'Diazepan',
                     'Ansiopax',
                    ...
                    ['Keflex', 'Ceporex', 'Dermaciclina',
                        'Cefalexina EMS', 'Alektos'],
...
print('VALUES')
print(',\n'.join(f'({i}, {repr(pa)})' for i,
      pa in enumerate(principios_ativos)) + ';')
...
dosagens = produto_cartesiano(
    ['1/4 cp', '1/2 cp', '1 cp', '2 cp', '3 cp', '4 cp', '6 cp',
        '1 gota', '5 gotas', '10 gotas', '30 gotas', '12 gotas'],
    ['', ' sublingual'],
    [' todos os dias', ' em dias alternados',
        ' semanalmente', ' a cada três dias'],
    [' após as refeições', ' antes de dormir',
        ' depois do almoço', ' de 6 em 6h', ' de 8 em 8h']
)
...
print('VALUES')
print(',\n'.join((f'({i}, {repr(n)}, {repr(v)})' for i,
      (n, v) in enumerate(vars_exames_quant))) + ';\n')
...
cond_coop = [x.strip() for x in ['Paciente não realizou o jejum indicado antes do exame de sangue.  ',
    ...
    if am:
        ...
        if esp == 2:
            ...
            for m in range(rng.randint(1, 4)):
                ...
                arquivos.append(
                    f'({arqid}, {repr("exame-{}.{}".format(arqid, ta.lower()))}, {repr(ta)}, "teste")')
        ...
        if esp == 2:
            ...
            arquivos.append(
                f'({arqid}, {repr("laudo-{}.{}".format(arqid, ta.lower()))}, {repr(ta)}, "teste")')
